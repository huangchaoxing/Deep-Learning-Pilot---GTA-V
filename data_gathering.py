...
class DogCatdataset(Dataset):
     def __init__(self,image_dir,train):
         ...
         if train == 1:
             ...
             self.sub_dir_list=self.dir_list[int(0.8*num):]
             self.trans=transforms.Compose([ transforms.Resize((100,100)),transforms.ToTensor(),normalization])
...
#       transforms.ToTensor(),normalization])
    ...
    training_loader=torch.utils.data.DataLoader(training_set,batch_size=16,num_workers=0)
    validation_loader=torch.utils.data.DataLoader(validation_set,batch_size=16,num_workers=0)
