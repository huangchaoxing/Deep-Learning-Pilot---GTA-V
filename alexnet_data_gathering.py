...
class DogCatdataset(Dataset):
     def __init__(self,image_dir,train):
         ...
         if train == 1:
             self.sub_dir_list=self.dir_list[:int(0.9*num)]
             ...
             self.sub_dir_list=self.dir_list[int(0.9*num):]
             self.trans=transforms.Compose([ transforms.Resize((224,224)),transforms.ToTensor(),normalization])
...
#       transforms.ToTensor(),normalization])
    ...
    training_loader=torch.utils.data.DataLoader(training_set,batch_size=32,num_workers=0)
